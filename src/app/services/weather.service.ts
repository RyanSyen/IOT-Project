...
// import { map, filter } from 'rxjs/operators'; //! Make sure that you always import from rxjs and never from rxjs/internal or even rxjs/internal/Observable
import { map, filter } from 'rxjs';
import { first } from 'rxjs/operators'; // Operators like first() must be imported from rxjs/operators
...
export class WeatherService {
  ...
  public getWeatherData(id: number): Observable<IWeatherData | undefined> {
    return this.weatherData!.asObservable()!.pipe(
      map((data: IWeatherData[]) => data!.find((weatherData: IWeatherData) => weatherData.id === id))
    );
  }

  public getForecastData(id: number): Observable<IForecast | undefined> {
      ...
      .subscribe((weatherData: IWeatherData) => {
        if (config.byCoordinates) {
          if (this.currentCoordinatesLocationId === -1) {
            const id = weatherData.id !== undefined ? weatherData.id : 0; // 👈️ type guard
            this.selectedLocationId.next(id);
            // this.selectedLocationId.next(weatherData.id);
          }
          this.currentCoordinatesLocationId = weatherData.id!; //👈️ non null assertion
        }
